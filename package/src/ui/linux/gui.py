...
class BaseUI():
    ...
    def create_menu(self, window):
        ...
        menu_items = (
            ...
            ("/File/_Settings", "<control>S", self.settings_clicked, 0, None),
            ("/File/_About", "<control>A", self.about_clicked, 0, None),
            ("/File/_Refresh", "<control>R", self.refresh_clicked, 0, None),
            )
        ...
        # generate the menu items from our list
        item_factory.create_items(menu_items)

        # attach the new accel groups to the window
        window.add_accel_group(accel_group)

        # keep ref so it doesn't get destroyed
        self.item_factory = item_factory

        # return the menu bar made by item_factory
        return item_factory.get_widget("<main>")

    def settings_clicked(self, button, menu):

        # the window is the grandparent of this menu
        # (don't ask me why it's not the parent)
        window = menu.get_parent().get_parent()
    ...
    def about_clicked(self, button, menu):
...
class mEveMonUI(BaseUI):
    ...
    def run(self):
        ...
        main_vbox.pack_start(menu, False, True, 0)
        menu.show()
    ...
    def refresh_clicked(self, button, menu):
        self.char_model.get_characters()
        # wait notification end --danny
...
class CharacterSheetUI(BaseUI):
    ...
    def build_window(self):
        ...
        container_vbox = gtk.VBox(False, 1)
        scrolled_window = gtk.ScrolledWindow()
        scrolled_window.add_with_viewport(vbox)
        container_vbox.pack_start(scrolled_window, True, True, 0)
        self.win.add(container_vbox)
        ###self.win.add(vbox)        
